...
function phaseTone(status) {
  ...
  if (status === 'done') {
    return 'text-zinc-500 border-zinc-700/60 bg-zinc-900/30';
  }

  return 'text-zinc-300 border-zinc-700/70 bg-zinc-900/40';
}
...
export default function Live() {
  ...
  const currentItem = timeline.find((item) => now >= item.startAt && now < item.endAt) || null;
  ...
  const mainHeadline = eventFinished
    ? 'TIME IS UP'
    : currentItem
  ...
  const subHeadline = eventFinished
  ...
  return (
    <div className="min-h-screen bg-[#02030b] text-white">
      <section className="relative overflow-hidden px-4 pb-14 pt-22 md:px-6 md:pt-24">
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_10%,rgba(45,212,191,0.11),transparent_34%),radial-gradient(circle_at_80%_8%,rgba(37,99,235,0.2),transparent_30%),linear-gradient(to_bottom,rgba(2,4,18,0.94),rgba(2,4,18,1))]" />
        ...
        <div className="relative mx-auto max-w-7xl">
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/10 bg-[#05081a]/80 px-4 py-3 backdrop-blur-md">
            <div className="flex items-center gap-2 text-xs uppercase tracking-[0.16em] text-zinc-400">
              <span className="h-2 w-2 rounded-full bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,0.9)]" />
              Live system synced
            </div>
            <div className="text-xs uppercase tracking-[0.15em] text-amber-300">Main portal</div>
          </div>

          <h1 className="mb-6 text-3xl font-black uppercase leading-tight tracking-[0.05em] text-transparent md:text-6xl md:leading-[1.05] bg-gradient-to-r from-amber-300 via-slate-200 to-emerald-400 bg-clip-text">
            Founders Pit Live 2026
          </h1>

          <div className="grid gap-6 lg:grid-cols-[1.7fr_0.9fr]">
            <div className="rounded-3xl border border-red-500/70 bg-[#030617]/90 p-4 shadow-[0_0_0_1px_rgba(239,68,68,0.15),0_24px_80px_rgba(0,0,0,0.7)] md:p-6">
              <div className="rounded-full border border-red-400/40 bg-red-500/15 px-4 py-1 text-center text-[10px] font-semibold uppercase tracking-[0.3em] text-red-200 md:inline-flex">
                {eventFinished ? 'Mission concluded' : 'Mission in progress'}
              </div>

              <div className="mt-8 rounded-2xl border border-red-400/25 bg-[#040a24] px-4 py-8 text-center md:px-10 md:py-14">
                <p className="text-xs uppercase tracking-[0.2em] text-zinc-400">Current phase</p>
                <h2 className={`mt-3 text-4xl font-black uppercase tracking-[0.04em] md:text-7xl ${eventFinished ? 'text-red-400' : 'text-emerald-300'}`}>
                  {mainHeadline}
                </h2>
              ...
              <div className="mt-5 grid gap-3 md:grid-cols-3">
                <div className="rounded-xl border border-white/10 bg-[#070b1f] p-3">
                  ...
                  <p className="mt-1 text-xl font-semibold text-cyan-300">{formatClock(now)}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-[#070b1f] p-3">
                  <p className="text-[11px] uppercase tracking-[0.15em] text-zinc-500">Countdown</p>
                  <p className={`mt-1 text-xl font-semibold ${eventFinished ? 'text-red-300' : 'text-amber-200'}`}>
                ...
                <div className="rounded-xl border border-white/10 bg-[#070b1f] p-3">
                  ...
                  <p className={`mt-1 text-sm font-semibold uppercase tracking-[0.15em] ${eventFinished ? 'text-red-300' : 'text-emerald-300'}`}>
                    {phaseHeader}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-5">
                ...
                <div className="h-2.5 overflow-hidden rounded-full bg-white/10">
                  <div
                    className="h-full bg-gradient-to-r from-cyan-400 via-blue-400 to-emerald-400 transition-all duration-700"
            ...
            <div className="space-y-5">
              <div className="rounded-3xl border border-white/10 bg-[#05081a]/85 p-5 backdrop-blur-md">
                <p className="text-[11px] uppercase tracking-[0.16em] text-zinc-400">Current phase</p>
                <h3 className={`mt-2 text-3xl font-black uppercase leading-tight ${eventFinished ? 'text-red-400' : 'text-emerald-300'}`}>
                  {eventFinished ? 'Event concluded' : currentItem ? currentItem.title : 'Awaiting launch'}
                </h3>
                  ...
                  {timeline.map((item) => {
                    ...
                    return (
                      <div key={`${item.title}-${item.start}`} className="flex items-start gap-3">
                        <div className={`mt-1 h-3 w-3 rounded-full border ${isLive ? 'border-emerald-300 bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.95)]' : isDone ? 'border-zinc-700 bg-zinc-700' : 'border-zinc-500 bg-transparent'}`} />
